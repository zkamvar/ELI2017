% https://tex.stackexchange.com/a/34060/77699
...
\subsection{Training Objective 1: Research Training Plan}
...
My research has also largely focused on simple sequence repeat (SSR) data, and I have experience with genome assembly gained by creating a make-based genome assembly pipeline (\url{https://github.com/zkamvar/read-processing}). 
...
In addition, no previous studies have evaluated the population genomic variation of this fungal pathogen, despite more than 50 population studies to date using simple sequence repeat (SSR) markers, which have been shown to poorly resolve haplotype diversity \citep{lehner2017independently}. 
...
\begin{wrapfigure}[]{R}[]{0pt} % 8 if hanging on new page, 29 otherwise
...
For over 15 years, population genetic analysis on \textit{S. sclerotiorum} has been performed using the same set of SSR markers \citep{sirjusingh2001characterization}. 
...
\begin{wrapfigure}[16]{r}[]{0pt} % [lineheight]{position}[overhang]{width}
...
\noindent \textbf{Methods -}
The course duration will be one full semester (16 weeks). 

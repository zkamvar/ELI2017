...
\noindent \textbf{Methods} A total of eight subpopulations of \textit{S. sclerotiorum} will be gathered from canola fields in the Chinese provinces of Gansu, Qinghai, Anhui, and Hunan \citep{zhou2014dimethachlon} and North America (USA and Mexico) representing four populations from temperate regions (between $40^{\circ}N$ and $40^{\circ}N$ $66^{\circ}N$) and four populations from subtropical regions (between $23.5^{\circ}N$ and $40^{\circ}N$). Whole genomic DNA will be sequenced to >30X depth on Illumina HiSeq 4000 sequencer and deposited in the UNL Attic data storage system. All isolates will be mapped to the \textit{S. sclerotiorum} reference genome using a workflow modified from \url{https://github.com/zkamvar/read-processing}, which uses \textsc{Bowtie2} and \textsc{Samtools} to map reads and \textsc{GATK} for variant discovery \citep{langmead2012fast, li2009sequence, mckenna2010genome, derbyshire2017complete}.
\noindent \textbf{Expectations}
\noindent \textbf{Pitfalls and Limitations}
% \subsubsection{Objective \#2: Common Garden Experiment to test for Local Adaptation}
